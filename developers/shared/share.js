...
(function(){
  ...
  function paintState(el){ var r=get(S); el.textContent=r?'Shared with '+r.name+'.':'' }
  ...
  function sentCopy(r){ var what=r.note&&r.inc?'Your note and your '+r.inc+' sandbox '+(r.inc===1?'request':'requests')+' go with it.':r.note?'Your note goes with it.':r.inc?'Your '+r.inc+' sandbox '+(r.inc===1?'request goes':'requests go')+' with it.':''; return '<div class="dv-share-sent" tabindex="-1"><b>Sent to '+esc(r.name)+'.</b><p>'+esc(r.email)+' gets a link to '+(r.from==='certification'?'this guide':'this reference')+', the two-page overview, and a deal page the network prepares for your company.'+(what?' '+what:'')+'</p></div>' }
  ...
  window.dvShare=function(from,btn){ if(window.RF&&RF.tryClose) RF.tryClose(); opener=btn||document.activeElement; if(box) close(); if(!scrim){ scrim=document.createElement('div'); scrim.className='gn-scrim'; scrim.addEventListener('click',close); document.body.appendChild(scrim) }
    ...
    box.innerHTML='<div class="gn-modal-h"><h2 id="shTitle">Share with the person who signs</h2><button class="gn-panel-x" type="button" aria-label="Close" data-sh-close>×</button></div>'
     +'<div class="gn-modal-b"><p>They get a link to '+(from==='certification'?'this guide':from==='reference'?'this reference':'the sandbox and the reference')+', the two-page overview, and a deal page the network prepares for your company. No account is created for either of you.</p>'
     ...
     +'<label class="gn-field"><span class="gn-field-l">Their email</span><input class="gn-in" id="shEmail" placeholder="name@acquirer.example" autocomplete="off" inputmode="email"></label>'
     +'<label class="gn-field"><span class="gn-field-l">A note <small>Optional</small></span><textarea class="gn-in" id="shNote" rows="3"></textarea></label>'
    ...
    box.querySelector('#shForm').addEventListener('submit',function(e){ e.preventDefault(); var n=box.querySelector('#shName'), m=box.querySelector('#shEmail'); if(dvOffline(m)) return;
      var a=dvErr(n, n.value.trim()?'':'Enter their name.'); var b=dvErr(m, !m.value.trim()?'Enter their email.':!dvEmail(m.value)?'Enter an email address, like name@acquirer.example.':''); if(!(a&&b)){ dvFirstErr(box); return }
      var r={name:n.value.trim(), email:m.value.trim(), note:!!box.querySelector('#shNote').value.trim(), inc:box.querySelector('#shInc').checked&&s?s.n:0, from:from}; set(S,r);
      ...
      dvToast('Sent to '+r.name); window.dvShareState() });
  ...
  window.dvWon=function(el, from){ if(!el) return; var c=el.closest('.gn-console')||el.parentNode, s=c.parentNode.querySelector('.dv-won'); if(!s){ s=document.createElement('div'); s.className='gn-strip dv-gate dv-won'; s.setAttribute('role','status'); s.innerHTML='<b>It worked in the sandbox.</b><span>Certification and production need a participation agreement. Someone who can sign for your company signs it in the Partner Portal.</span><div class="gn-actions"><button class="gn-btn" type="button" data-share="'+from+'">Share with the person who signs</button><span class="gn-num" data-share-state></span></div>'; c.parentNode.insertBefore(s, c.nextSibling) } s.hidden=false; window.dvShareState() };
